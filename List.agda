...
module _ {ℓ}{A : Type ℓ} {C : Type ℓ} where
  ...
  recList m {Δ₀} {Δ₁} J f (x ∷ xs) = ap (prod₂ ⊸∘ prod₁) (ΠApp 1 (ΠApp m f (ax x)) (recList m J f xs))
    ...
    prod₁ = ((⊗^ m) ⊸∘ (⊸^ m (ι∷ {x = x} {xs = xs}))) ⊸⊗ (id ((Δ₁ ⊗ Δ₁ ^ length xs) ⊗ Δ₀))

    prod₂ : (η x ^ m ⊗ η xs ^ m) ⊗ (Δ₁ ⊗ Δ₁ ^ length xs) ⊗ Δ₀ ⊸
      (Δ₁ ⊗ η x ^ m) ⊗ (η xs ^ m ⊗ Δ₁ ^ length xs ⊗ Δ₀) ⊗ ◇
    prod₂ = id (Δ₁ ⊗ η x ^ m) ⊸⊗ unitr⊸' _ ⊸∘ assoc' Δ₁ (η x ^ m) (η xs ^ m ⊗ Δ₁ ^ length xs ⊗ Δ₀) ⊸∘ id Δ₁ ⊸⊗ assoc (η x ^ m) (η xs ^ m) (Δ₁ ^ length xs ⊗ Δ₀) ⊸∘ assoc (Δ₁) (η x ^ m ⊗ η xs ^ m) (Δ₁ ^ length xs ⊗ Δ₀)  ⊸∘ (swap (η x ^ m ⊗ η xs ^ m) Δ₁) ⊸⊗ (id $ Δ₁ ^ length xs ⊗ Δ₀) ⊸∘ assoc' (η x ^ m ⊗ η xs ^ m) Δ₁ (Δ₁ ^ length xs ⊗ Δ₀) ⊸∘ (id (η x ^ m ⊗ η xs ^ m) ⊸⊗ assoc Δ₁ (Δ₁ ^ length xs) Δ₀)
