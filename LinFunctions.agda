...
module _ {Δ : Supply ℓ} {(A , Θ₀) : LType ℓ} {(B , Θ₁) : (A , Θ₀) ⇒ LType} where
  ΠI : ((x : A) → Δ ⊗ Θ₀ x ⊩ B x , Θ₁) → Δ ⊩ ⟨ x ∶ A , Θ₀ ⟩ ⊸ B x , Θ₁
  ΠI fδ = (λ x → fst (fδ x)) , bind (λ x → curry (snd (fδ x)))

  infixr -5 ΠI
  syntax ΠI (λ x → b) = 𝛌 x ↦ b

module _ {Δ : Supply ℓ} {(A , Θ₀) : LType ℓ} {(B , Θ₁) : (A , Θ₀) ⇒ LType} where
  ΠIβ : ((x : of (A , Θ₀)) → Δ ⊗ Θ₀ (x .term) ⊩ B (x .term) , Θ₁) → Δ ⊩ ⟨ x ∶ A , Θ₀ ⟩ ⊸ B x , Θ₁
  ΠIβ fδ = (λ x → fδ (- x) .fst ) , bind (λ x → curry (fδ (- x) .snd))

  infixr -5 ΠIβ
  syntax ΠIβ (λ x → b) = 𝛌 x ↦𝕗 b


．ΠI : {Δ : Supply ℓ} {A : Set ℓ} {(B , Θ) : (A ．) ⇒ LType}
  → ((x : A) → (Δ ⊗ ι x) ⊩ B x , Θ)
  → Δ ⊩ ⟨ x ∶ A ． ⟩ ⊸ B x , Θ
．ΠI {ℓ} {Δ} {A} = ΠI {ℓ} {Δ} {A , ι}
...
module _ {Δ₀ Δ₁ : Supply ℓ} {(A , Θ₀) : LType ℓ} {(B , Θ₁) : (A , Θ₀) ⇒ LType} where
  ...
  (f , δ₀) ﹫ (a , δ₁) = f a , uncurry (free δ₀ a) ∘ (id Δ₀ ⊗ᶠ δ₁)
...
_﹫．_ : {Δ₀ Δ₁ : Supply ℓ} {(A , Θ₀) : LType ℓ} {B : A → Type ℓ}
  ...
  → ((a , _) : Δ₁ ⊩ A , Θ₀)
  → (Δ₀ ⊗ Δ₁ ⊩ B a ．)
_﹫．_ {_} {Δ₀} (b , f) (a , δ) = b a , uncurry (free f a) ∘ id Δ₀ ⊗ᶠ δ
